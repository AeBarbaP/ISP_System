...
function querySolicitudes() {
  ...
  modal.classList.add('modal', 'fade');
  modal.setAttribute('tabindex', '-1');
  modal.setAttribute('id', 'modalSolicitudesPagos');
  modal.innerHTML = `
    <div class="modal-dialog modal-xl">>
      <div class="modal-content">
          <div class="modal-header">
          ...
          <div class="modal-body">
              <input name="id" id="idHiddenSolicitudes" value="" hidden>
              <div class="row">
                <div class="col-6">
                  <div class="input-group mb-3">
                      <span class="input-group-text" id="basic-addon1"><i class="bi bi-calendar-check-fill"></i></span>
                      <input type="date" value="${fecha}" class="form-control" aria-describedby="basic-addon1" id="buscarSolicitudesDFecha" name="buscar" onchange="datosSolicitudesDiarios()">
                  </div>
                </div>
                <div class="col-6">
                  <div class="input-group mb-3">
                      <span class="input-group-text" id="basic-addon1"><i class="bi bi-search"></i></span>
                      <input type="text" class="form-control" placeholder="Buscar..." aria-label="Buscar" aria-describedby="basic-addon1" id="buscarSolicitudesDPagos" name="buscar">
                  </div>
                </div>
              </div>
              <div class="table-responsive mt-3">
                  <table class="table table-hover text-center p-1">
                      ...
                      <tbody id="tablaSolicitudesCajaReporte">
  ...
  const bootstrapModal = new bootstrap.Modal(modal);
  bootstrapModal.show();
  datosSolicitudesDiarios();

    $(document).ready(function () {
    $("#buscarSolicitudesDPagos").on("keyup", function () {
    var value = $(this).val().toLowerCase();
    $("#tablaSolicitudesCajaReporte tr").filter(function () {
        $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
      });
  });
...
function datosSolicitudesDiarios(){
  let fecha = _('buscarSolicitudesDFecha').value;
  $.ajax({
    url: 'query/query_solicitudesDiarios.php',
    type: 'POST',
    data:{
      fecha : fecha
    },
    dataType: 'html',
    success: function (response) {
        $('#tablaSolicitudesCajaReporte').html(response);
    }
  });
}